...
class Dosen_model extends CI_Model {
  ...
  public function getData( $criteria = '', $page = 1, $limit = 20, &$total_rows = 0) {
	...
	public function getDosenAktivitas(){
		//retrieve all
		$this->db->select('a.nm_ptk,a.nidn,a.jk,a.tmpt_lahir,a.tgl_lahir,d.id_jenj_didik,g.nm_mk,g.sks_mk,g.sks_tm,g.sks_prak_lap');
		$this->db->from('ptk a');
		$this->db->join('satuan_pendidikan b','a.id_sp = b.id_sp','inner');
		$this->db->join('lembaga_non_sp c','c.id_wil = b.id_wil','left');
		$this->db->join('rwy_pend_formal d','d.id_ptk = a.id_ptk','inner');
		$this->db->join('rwy_fungsional e','e.id_ptk = a.id_ptk','inner');
		$this->db->join('sms f','f.id_sp = a.id_sp','inner');
		$this->db->join('matkul g','g.id_sms = f.id_sms','inner');
		$this->db->join('matkul_kurikulum h','h.id_mk = g.id_mk','inner');
		$this->db->limit(300);
